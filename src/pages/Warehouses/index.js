...
const Warehouses = () => {
  ...
  const moveMutation = useMutation((data) => {
  ...
  const handleSearchProductions = async (input, cb) => {
    const productions = await fetch(BASE_URL + '/productions?q=' + input).then(r => r.json())
  ...
  const handleMoveSelectChange = (e, p) => {
    setProductionsToMove([
      ...productionsToMove,
      { warehouse_id: e.value, amount: 0, production_id: p.production_id, warehouse_name: e.label }
    ])
  }

  const handleDistributedAmountChange = (e, p) => {
    const productionsIndex = warehouse.productions.findIndex(i => i.production_id === p.production_id)
  ...
  const handleEdit = async id => {
    const warehouse = await fetch(BASE_URL + '/warehouses/' + id).then(res => res.json())
  ...
  const handleWarehouse = useCallback(() => {
    if (isMove && !isDelete) {
      const preparedMoveTo = productionsToMove.map(({warehouse_id, production_id, amount}) => {
        return {
          warehouse_id,
          production_id,
          amount
    ...
    isEdit ? updateMutation.mutate(warehouse) :
  ...
  return (
    <section className="section">
      <div className="container">
                      ...
                      <tr key={warehouse.id}>
                        ...
                        <td className="is-actions-cell">
                          <div className="buttons is-right">
                            <button onClick={() => handleEdit(warehouse.id)} className="button is-small is-primary" type="button">
                              <span className="icon"><i className="fa fa-pen" /></span>
                            </button>
                            <button onClick={() => handleDelete(warehouse.id)} className="button is-small is-danger jb-modal" data-target="sample-modal" type="button">
                              <span className="icon"><i className="fa fa-trash" /></span>
                            </button>
                            <button onClick={() => handleMove(warehouse.id)} className="button is-small is-info jb-modal" data-target="sample-modal" type="button">
                              <span className="icon"><i className="fa fa-arrows-alt-h" /></span>
                            </button>
                          </div>
                        </td>
                      </tr>
        ...
        <Modal isDelete={isDelete} onSave={handleWarehouse} onClose={handleModalClose} isOpen={isModalOpen} isLoading={isLoading}>
          {!isDelete && !isMove ? (
            <>
              <form>
                  ...
                  <div className="column">
                    <div className="field">
                      ...
                      <div className="control">
                        <input name='name' value={warehouse.name} onChange={handleChange} className="input" type="text" placeholder="warehouse name" />
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div key={p.production_name} className="columns">
                    <div className="column is-8">
                      <div className="field">
                        ...
                        <div className="control">
                          <input name='productionname' value={`${p.production_name} ${p.unDistributedAmount === null ? '' : `(${p.unDistributedAmount} allowed)`}`} disabled className="input disabled" type="text" />
                        </div>
                      </div>
                    </div>
                    <div className="column">
                      <div className="field">
                        ...
                        <div className="control">
                          <input name='amount' onChange={e => handleDistributedAmountChange(e, p)} min={0} max={p.unDistributedAmount} defaultValue={p.amount} className="input" type="number" placeholder='Amount' />
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              {showDistributeMenu ? (
                <div className='mt-2'>
                  <AsyncSelect onChange={handleSelectChange} cacheOptions loadOptions={handleSearchProductions} />
                </div>
              ) : (
                  <button onClick={() => setShowDistributeMenu(true)} className="button is-link is-outlined mt-2">Add production</button>
                )}
            </>
          ) : (
              <div>
                {!isMove && (
                  <>
                    <div>{warehouse.name} contains {warehouse.productions.reduce((acc, val) => acc + val.amount, 0)} productions.</div>
                    <button onClick={() => setIsMove(true)} className="button is-info is-outlined mt-2">Move products</button>
                  </>
                )}
                {isMove && warehouse.productions.map((p, index) => (
                  <div key={p.production_id} className='columns'>
                      ...
                      <div className="columns">
                        ...
                        <div className="column">
                          <div className="field">
                            ...
                            <div className="control">
                              <input name='amount' value={p.amount} disabled className="input disabled" type="number" placeholder='Amount' />
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className='column'>
                      {productionsToMove.length && productionsToMove.findIndex(i => i.production_id === p.production_id) === index ? (
                        <div>
                          <div className="columns">
                            <div className="column is-5">
                              <div className="field">
                                <label className="label">name</label>
                                <div className="control">
                                  <input name='productionname' value={productionsToMove[index].warehouse_name} disabled className="input disabled" type="text" />
                                </div>
                              </div>
                            </div>
                            <div className="column">
                              <div className="field">
                                <label className="label">amount</label>
                                <div className="control">
                                  <input name='amount' value={productionsToMove[index].amount} onChange={e => handleAmountToMove(e, index)} className="input" type="number" placeholder='Amount' />
                                </div>
                              </div>
                            </div>
                          </div>
                          ...
                          <AsyncSelect onChange={e => handleMoveSelectChange(e, p)} cacheOptions loadOptions={handleSearchWarehouses} />
                        )}
                    </div>
                  </div>
                )
