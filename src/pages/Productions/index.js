...
const Productions = () => {
  ...
  const handleProduction = useCallback(() => {
    isEdit ? editMutation.mutate(production) : createMutation.mutate(production)
  }, [production])
  ...
  const handleDistributedAmountChange = (e, p) => {
    const applyToIndex = production.applyTo.findIndex(
      i => i.warehouse_id === p.warehouse_id
    )
  ...
  const handleEdit = async id => {
    const production = await fetch(BASE_URL + '/productions/' + id).then(r =>
      r.json()
    )
                    ...
                    <tr key={production.id}>
                      ...
                      <td className="is-actions-cell">
                        <div className="buttons is-right">
                          ...
                          <button
                            onClick={() => removeProducion(production)}
              ...
              <div className="column">
                <div className="field">
                  <label className="label">
                    Amount ({allowedAmountToDistribute})
                  </label>
                  <div className="control">
                    <input
                      name="amount"
